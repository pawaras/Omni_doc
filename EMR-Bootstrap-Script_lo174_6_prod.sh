...
aws s3 cp s3://2mpv-lo174-configurations-logs-s3/package/css_data_alr_package.tar css_data_alr_package.tar
...
sudo python3 -m pip   install wheel==0.41.3 -i ${PYPI_INDEX}
...
aws s3 cp s3://2mpv-lo174-configurations-logs-s3/jars/${POSTGRES_JAR} ${POSTGRES_JAR}
aws s3 cp s3://2mpv-lo174-configurations-logs-s3/jars/${SQLSERVER_JAR} ${SQLSERVER_JAR}
aws s3 cp s3://2mpv-lo174-configurations-logs-s3/jars/${ORACLE_JAR} ${ORACLE_JAR}
aws s3 cp s3://2mpv-lo174-configurations-logs-s3/jars/${SAP_JAR} ${SAP_JAR}
aws s3 cp s3://2mpv-lo174-configurations-logs-s3/jars/${REDSHIFT_JAR} ${REDSHIFT_JAR}

aws s3 cp s3://2mpv-lo174-configurations-logs-s3/jars/${COBOL_PARSER} ${COBOL_PARSER}
aws s3 cp s3://2mpv-lo174-configurations-logs-s3/jars/${SCODEC_BITS} ${SCODEC_BITS}
aws s3 cp s3://2mpv-lo174-configurations-logs-s3/jars/${SCODEC_CORE} ${SCODEC_CORE}
aws s3 cp s3://2mpv-lo174-configurations-logs-s3/jars/${SPARK_COBOL} ${SPARK_COBOL}
...
aws s3 cp s3://2mpv-lo174-configurations-logs-s3/package/sftp.sh /home/hadoop
chmod 755 /home/hadoop/sftp.sh

aws s3 cp s3://2mpv-lo174-configurations-logs-s3/package/ttt /home/hadoop/.ssh/id_rsa
aws s3 cp s3://2mpv-lo174-configurations-logs-s3/package/t_hosts /home/hadoop/.ssh/known_hosts
